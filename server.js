...
function allColorsUsed(r, exceptId=null) {
  ...
  for (const p of r.players.values()) {
    if (p.id !== exceptId) used.add(p.color);
  }
  return COLORS.every(c => used.has(c.key));
}
function colorUsed(r, key, exceptId=null) {
  // Normally every color can be selected only once.
  // When all colors are already occupied, duplicate colors become allowed.
  if (allColorsUsed(r, exceptId)) return false;
  for (const p of r.players.values()) {
    if (p.id !== exceptId && p.color === key) return true;
  }
  return false;
}
...
function safeRoom(r) {
  return {
    code:r.code, hostId:r.hostId, status:r.status, tasks:r.tasks, impostorCount:r.impostorCount,
...
wss.on('connection',ws=>{
  ...
  ws.on('message',raw=>{
    ...
    if(m.type==='create' || m.type==='join'){
      ...
      let r;
      if(m.type==='create'){
        r={code:roomCode(),hostId:null,status:'lobby',tasks:[],impostorCount:1,players:new Map(),airUsed:false,airCount:0,warning:null,voting:null,result:null,winner:null};
        const pid=id();
        r.hostId=pid;
    ...
    if(m.type==='setImpostors'){
      if(r.hostId!==me.id || r.status!=='lobby') return;
      const count=Math.max(1,Math.min(3,Math.floor(Number(m.count)||1)));
      if(r.players.size < count + 1){
        return send(ws,{type:'error',message:`Na ${count} impostorov treba aspoň ${count+1} hráčov.`});
      }
      r.impostorCount=count;
      return broadcastRoom(r);
    }
    ...
    if(m.type==='start'){
      ...
      shuffled.slice(0,r.impostorCount).forEach(p=>p.role='Impostor');
